...
const AdminDashboard: React.FC = () => {
  ...
  const [ridePage, setRidePage] = useState<number>(1);
  const [totalUserPages, setTotalUserPages] = useState<number>(1);
  const [totalRidePages, setTotalRidePages] = useState<number>(1);
  ...
  const [userCampusFilter, setUserCampusFilter] = useState<string>('all');
  ...
  const [rideDirectionFilter, setRideDirectionFilter] = useState<string>('all');
  const [rideVehicleTypeFilter, setRideVehicleTypeFilter] = useState<string>('all');
  const [rideSortBy, setRideSortBy] = useState<string>('dateDesc');
  const [rideDateFrom, setRideDateFrom] = useState<string>('');
  const [rideDateTo, setRideDateTo] = useState<string>('');
  ...
  // Helper function removed - now using getDisplayName from utils
  ...
  // Refetch when user filters change
  useEffect(() => {
    if (currentUser?.isAdmin && activeTab === 'users') {
      fetchData(1); // Reset to page 1 when filters change
    }
  }, [userCampusFilter, userRoleFilter, userSortBy]);

  // Refetch when ride filters change
  useEffect(() => {
    if (currentUser?.isAdmin && activeTab === 'rides') {
      fetchData(1); // Reset to page 1 when filters change
    }
  }, [rideStatusFilter, rideDirectionFilter, rideVehicleTypeFilter, rideSortBy, rideDateFrom, rideDateTo]);

  // Check maintenance mode status
  const checkMaintenanceMode = async () => {
    try {
      const response = await api.get('/api/admin/maintenance-mode');
      if (response.data.success) {
        setIsMaintenanceMode(response.data.isActive);
      }
    } catch (error) {
  ...
  const fetchData = async (page?: number) => {
    try {
      setIsLoading(true);
      if (activeTab === 'users') {
        const currentPage = page || userPage;
        const params = new URLSearchParams({
          page: currentPage.toString(),
          limit: '50',
          campus: userCampusFilter,
          role: userRoleFilter,
          sortBy: userSortBy
        });
        ...
        setTotalUserPages(response.data.pagination.totalPages);
        setTotalUsers(response.data.pagination.totalUsers);
      } else if (activeTab === 'rides') {
        const currentPage = page || ridePage;
        const params = new URLSearchParams({
          page: currentPage.toString(),
          limit: '50',
          status: rideStatusFilter,
          direction: rideDirectionFilter,
          vehicleType: rideVehicleTypeFilter,
          sortBy: rideSortBy
        });
        if (rideDateFrom) params.append('dateFrom', rideDateFrom);
        if (rideDateTo) params.append('dateTo', rideDateTo);
        const response = await api.get(`/api/admin/rides?${params}`);
        setRides(response.data.rides);
        setFilteredRides(response.data.rides);
        setRidePage(currentPage);
        setTotalRidePages(response.data.pagination.totalPages);
        setTotalRides(response.data.pagination.totalRides);
  ...
  const goToRidePage = (page: number) => {
    if (page >= 1 && page <= totalRidePages) {
      fetchData(page);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-3">
            ...
            >
              {isMaintenanceMode ? '✓ Maintenance ON' : 'Maintenance Mode'}
            </button>
        ...
        {isLoading ? (
          ...
          <>
              ...
              <div className="bg-white shadow rounded-lg p-6">
                ...
                <div className="flex flex-wrap gap-4 mb-6">
                  ...
                  >
                    ...
                    <option value="PES University Ring Road Campus">RR Campus</option>
                    <option value="PES University Electronic City Campus">EC Campus</option>
                  </select>
                  ...
                  >
                    ...
                    <option value="driver">Active as Driver</option>
                    <option value="hitcher">Active as Hitcher</option>
                  </select>
                  ...
                  {(userCampusFilter !== 'all' || userRoleFilter !== 'all' || userSortBy !== 'newest') && (
                    <button
                      onClick={() => {
            ...
            {activeTab === 'rides' && (
              <div className="bg-white shadow rounded-lg p-6">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-semibold">All Rides ({totalRides})</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                  ...
                  <select
                    value={rideDirectionFilter}
                    onChange={(e) => setRideDirectionFilter(e.target.value)}
                    className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                  ...
                  <select
                    value={rideVehicleTypeFilter}
                    onChange={(e) => setRideVehicleTypeFilter(e.target.value)}
                    className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                  ...
                  <select
                    value={rideSortBy}
                    onChange={(e) => setRideSortBy(e.target.value)}
                    className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    ...
                    <option value="seatsDesc">Seats (Most First)</option>
                    <option value="seatsAsc">Seats (Least First)</option>
                  </select>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="date"
                      value={rideDateFrom}
                      onChange={(e) => setRideDateFrom(e.target.value)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="date"
                      value={rideDateTo}
                      onChange={(e) => setRideDateTo(e.target.value)}
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                ...
                {/* Clear Filters Button */}
                {(rideStatusFilter !== 'all' || rideDirectionFilter !== 'all' || rideVehicleTypeFilter !== 'all' || rideDateFrom || rideDateTo || rideSortBy !== 'dateDesc') && (
                  <div className="mb-4">
                    <button
                      onClick={() => {
                        setRideStatusFilter('all');
                        setRideDirectionFilter('all');
                        setRideVehicleTypeFilter('all');
                        setRideSortBy('dateDesc');
                        setRideDateFrom('');
                        setRideDateTo('');
                      }}
                      className="text-sm text-blue-600 hover:text-blue-800 underline"
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredRides.map(ride => (
                        <tr key={ride._id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {getDisplayName(ride.driver.name)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {ride.driver.srn || 'N/A'}
                          </td>
                ...
                {totalRidePages > 1 && (
                  <div className="mt-6 flex items-center justify-center gap-2">
                    ...
                    <button
                      onClick={() => goToRidePage(ridePage - 1)}
                      disabled={ridePage === 1}
                      className="px-3 py-1 rounded-md bg-gray-200 hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    ...
                    <span className="px-4 py-1 text-sm font-medium">
                      Page {ridePage} of {totalRidePages}
                    </span>
                    <button
                      onClick={() => goToRidePage(ridePage + 1)}
                    ...
                    <button
                      onClick={() => goToRidePage(totalRidePages)}
                      disabled={ridePage === totalRidePages}
                      className="px-3 py-1 rounded-md bg-gray-200 hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Last
                        ...
                        <tr key={issue._id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium">
                              {getDisplayName(issue.reporter.name)}
                            </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium">
                              {getDisplayName(issue.reportedUser.name)}
                            </div>
                            <div className="text-xs text-gray-500">
                              <span className={`inline-block px-2 py-0.5 rounded ${
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-700 mb-6">
              {isMaintenanceMode 
                ? 'Are you sure you want to disable maintenance mode? Users will be able to access the website again.'
                : 'Are you sure you want to enable maintenance mode? All users (except admins) will be unable to access the website and will see a maintenance page.'}
            </p>
            <div className="flex justify-end space-x-4">
              ...
              <LoadingButton
                ...
                loadingText={isMaintenanceMode ? 'Disabling...' : 'Enabling...'}
