...
const RideEdit: React.FC = () => {
  ...
  const handleUpdateRide = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.response?.data?.error || "Failed to update ride";
          ...
          <div className="space-y-4">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (newDate) {
                    ...
                    if (hasExistingRide) {
                      updatedErrors.direction = `You already have a ${rideForm.direction === "toCollege" ? "To College" : "From College"} ride scheduled on this date`;
                    } else {
            ...
            <div>
              ...
              {validationErrors.pricePerKm ? (
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {currentUser?.driverProfile?.vehicle?.type !== "scooter/bike" && 
                   rideForm.pricePerKm !== undefined && 
                   rideForm.availableSeats > 0 && (
                    <>
