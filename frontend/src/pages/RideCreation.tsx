...
const RideCreation: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    if (currentUser) {
      const vehicleType = currentUser.driverProfile?.vehicle?.type;
      const defaultSeats = vehicleType === "scooter/bike" ? 1 : (currentUser.driverProfile?.vehicle.seats || 4);
  ...
  const handleCreateSchedule = useCallback(async (): Promise<void> => {
    ...
    // Check 7-day limit (matching backend validation)
    const maxDate = new Date();
    maxDate.setDate(maxDate.getDate() + 7);
    maxDate.setHours(23, 59, 59, 999); // End of day, 7 days from now
    
    if (selectedDate > maxDate) {
      errors.date = "Cannot schedule rides more than 7 days in advance";
    }
    ...
    try {
      ...
      // Check if it's a date validation error from backend
      if (errorMessage.includes("more than one week") || errorMessage.includes("7 days")) {
        setValidationErrors({
          ...errors,
          date: "Cannot schedule rides more than 7 days in advance"
        });
      } else if (errorMessage.includes("past") || errorMessage.includes("date")) {
        setValidationErrors({
          ...errors,
          date: errorMessage
        });
      } else {
        // Show a generic error for other backend errors
        ...
        <div className="bg-white shadow-md rounded-lg p-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              {currentUser?.driverProfile?.vehicle?.type === "scooter/bike" ? (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  // Validate date immediately (important for iOS)
                  if (newDate) {
                    ...
                    if (selectedDate < todayDate) {
                      updatedErrors.date = "Cannot schedule rides for past dates";
                    } else {
                      // Check 7-day limit
                      const maxDate = new Date();
                      maxDate.setDate(maxDate.getDate() + 7);
                      ...
                      if (selectedDate > maxDate) {
                        updatedErrors.date = "Cannot schedule rides more than 7 days in advance";
                      } else {
                        // Date is valid, clear date error
                        delete updatedErrors.date;
                      }
                    }
                    
                    // Check if changing the date would conflict with existing rides
                    if (checkForExistingRide(newDate, ride.direction)) {
                      updatedErrors.direction = `You already have a ${ride.direction === "toCollege" ? "To College" : "From College"} ride scheduled on this date`;
                    } else {
                      delete updatedErrors.direction;
                    }
                  } else {
                    // Clear errors if date is empty
                    delete updatedErrors.date;
                    delete updatedErrors.direction;
                  }
