...
const HitcherDashboard: React.FC = () => {
  ...
  const filterRides = (rides: ExtendedRide[]): { upcomingRides: ExtendedRide[], pastRides: ExtendedRide[] } => {
    ...
    const upcomingRides = rides.filter((ride: ExtendedRide) => {
      ...
      // Exclude rides that are cancelled, rejected, or where the hitcher status is cancelled/cancelled-by-driver/auto-rejected
      if (
        ride.status === "cancelled" || 
        ride.status === "completed" || 
        hitcherInfo?.status === "cancelled" || 
        hitcherInfo?.status === "rejected" ||
        hitcherInfo?.status === "auto-rejected" ||
    ...
    const pastRides = rides.filter((ride: ExtendedRide) => {
      ...
      // Include rides that are cancelled/rejected/auto-rejected in past rides
      if (
        ride.status === "cancelled" || 
        ride.status === "completed" ||
        hitcherInfo?.status === "cancelled" || 
                            ...
                            {(hitcherInfo.status === 'accepted' || ride.status === 'in-progress') && 
                              ...
                              <div className="mt-2 mb-4 bg-blue-50 p-3 rounded-md">
                                ...
                                <div className="space-y-2">
                                  <p className="text-sm">
                                    <span className="font-medium">Name:</span> {getDisplayName(ride.driver.name)}
                                  </p>
                                  <p className="text-sm">
                                  ...
                                  {ride.driver.driverProfile?.vehicle && (
                                    <>
                                      <p className="text-sm">
                                        <span className="font-medium">Vehicle:</span> {ride.driver.driverProfile.vehicle.model}
                                      </p>
                                      <p className="text-sm">
                                        <span className="font-medium">Vehicle Type:</span> {ride.driver.driverProfile.vehicle.type}
                                      </p>
                                    </>
                                  )}
                                  <p className="text-sm">
