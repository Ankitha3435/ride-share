...
const Report: React.FC = () => {
  ...
  const [selectedHitcherId, setSelectedHitcherId] = useState<string>("");
              ...
              {activeTab === "upcoming" ? (
                          ...
                          <div className="space-y-2">
                            {ride.hitchers
                              ...
                              .map((hitcher) => (
                                ...
                                >
                                  <div>
                                    <p className="font-medium">{getDisplayName(hitcher.user.name)}</p>
                      ...
                      {currentUser.activeRoles.hitcher && (
                        <div className="mt-4">
                          <div className="flex items-center justify-between bg-gray-50 p-3 rounded-md">
                            <div>
                              <p className="font-medium">Driver: {getDisplayName(ride.driver.name)}</p>
                              <p className="text-sm text-gray-600">
                ...
                pastRides.map((ride) => (
                  ...
                  >
                    ...
                    {currentUser.activeRoles.driver && (
                      <div className="mt-4">
                        {/* Show hitchers who participated in the ride */}
                        {ride.hitchers && ride.hitchers.filter(h => h.status === "accepted").length > 0 && (
                          <div className="mb-3">
                            <h4 className="font-medium text-gray-900 mb-2">Hitchers who participated:</h4>
                            <div className="space-y-2">
                              {ride.hitchers
                                .filter((h) => h.status === "accepted")
                                .map((hitcher) => (
                                  ...
                                  >
                                    ...
                                    {!rideIssues[ride._id]?.[hitcher.user._id] && (
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          setSelectedRide(ride);
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              {/* For drivers with multiple hitchers, show a dropdown to select which one to report */}
              {currentUser.activeRoles.driver && selectedRide.hitchers && selectedRide.hitchers.filter(h => h.status === "accepted").length > 1 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700">Select Hitcher</label>
                  <select
                    value={selectedHitcherId}
                    onChange={(e) => setSelectedHitcherId(e.target.value)}
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                  >
                    <option value="">Select a hitcher...</option>
                    {selectedRide.hitchers
                      .filter(h => h.status === "accepted")
                      .map(hitcher => (
                        <option key={hitcher.user._id} value={hitcher.user._id}>
                          {getDisplayName(hitcher.user.name)}
                        </option>
                      ))}
                  </select>
                </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <LoadingButton
                  onClick={() => {
                    const reportedUserId = currentUser.activeRoles.driver
                      ? (selectedRide.hitchers && selectedRide.hitchers.filter(h => h.status === "accepted").length > 1
                          ? selectedHitcherId
                          : selectedRide.hitchers?.find(h => h.status === "accepted")?.user._id || "")
                      : selectedRide.driver._id;
                    handleReportIssue(selectedRide._id, reportedUserId);
                  }}
                  disabled={
                    ...
                    (currentUser.activeRoles.driver && 
                     selectedRide.hitchers && 
                     selectedRide.hitchers.filter(h => h.status === "accepted").length > 1 && 
                     !selectedHitcherId)
                  }
                  loadingText="Submitting..."
