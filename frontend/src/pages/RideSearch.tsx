...
const RideSearch: React.FC = () => {
  ...
  const [vehicleType, setVehicleType] = useState<"" | "scooter/bike" | "car">("");
  ...
  }, [allRides, currentUser, searchQuery, direction, selectedDate, minTime, maxTime, driverGender, vehicleType, ridesLoaded]);
  ...
  const handleRequestRide = async (rideId: string) => {
    ...
    try {
      ...
      if (selectedRide.availableSeats < 1) {
        setNotification({
          show: true,
          message: "This ride has no available seats left",
          type: "error",
        });
        setIsLoading(false);
        return;
      ...
      const hasAcceptedRide = allRides.some(ride => {
        ...
        // Check if the hitcher has an accepted request in this ride
        const hasAcceptedRequest = ride.hitchers?.some(
          h => h.user?._id === currentUser?.id && h.status === "accepted"
        );
        
        return isSameDay && isSameDirection && hasAcceptedRequest;
      });
      
      if (hasAcceptedRide) {
        setNotification({
          show: true,
          message: "You already have an accepted ride for this day and direction. You can't request multiple rides for the same trip.",
          type: "error",
        });
        setIsLoading(false);
        return;
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-lg font-semibold mb-4">Select Ride Preferences</h2>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="w-full sm:w-auto">
              ...
              <select
                value={vehicleType}
                onChange={(e) => setVehicleType(e.target.value as "" | "scooter/bike" | "car")}
                className="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
            ...
            <div className="w-full sm:w-auto">
              ...
              <input
                ...
                max={maxDateString}
                className="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                disabled={isLoading || !direction || !vehicleType}
                required
              />
            </div>
          </div>
          <p className="text-gray-500 text-sm mt-4">
            ...
            ) : !direction ? "Please select a direction first" : !vehicleType ? "Please select a vehicle type" : !selectedDate ? "Please select a date to continue" : "Showing rides for selected preferences"}
          </p>
        </div>
          ...
          <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 overflow-y-auto p-4">
            ...
            >
              ...
              {selectedRideDetails.direction === "toCollege" ? (
                <div className="mx-6 mt-2 text-red-600 text-lg font-bold">Your ride is likely to be accepted if your pickup point is on the way for the driver</div>
              ) : (
                <div className="mx-6 mt-2 text-red-600 text-lg font-bold">Your ride is likely to be accepted if your dropoff point is on the way for the driver</div>
              )}
              <div className="p-4">
                ...
                <div className="relative mb-8">
                  <MapPreview
                    ...
                    startCoordinates={selectedRideDetails.direction === "toCollege" ? selectedRideDetails.driver.homeCoordinates : undefined}
                    endCoordinates={selectedRideDetails.direction === "fromCollege" ? selectedRideDetails.driver.homeCoordinates : undefined}
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 rounded-md p-4">
                      ...
                      <p className="text-xs text-gray-500 italic mt-3">
                        Driver's name, phone and vehicle model name will be visible after request is accepted.
                      </p>
                    </div>
                  </div>
                </div>
