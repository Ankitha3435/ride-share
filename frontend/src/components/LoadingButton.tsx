...
const LoadingButton: React.FC<LoadingButtonProps> = ({
  ...
  return (
    <button
      ...
      style={preserveHeight ? { height: isLoading ? 'var(--original-height)' : 'auto' } as React.CSSProperties : undefined}
      ref={(el) => {
        if (el && preserveHeight && !isLoading) {
          // Store the original height for later use
          el.style.setProperty('--original-height', `${el.offsetHeight}px`);
        }
      }}
    >
      {isLoading ? (
