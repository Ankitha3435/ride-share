...
exports.updateDriverProfile = async (req, res) => {
  try {
    ...
    const { phone, homeAddress, homeCoordinates, driverProfile, gender, distanceToCollege } = req.body;
    ...
    // Validate and adjust based on vehicle type
    const vehicleType = driverProfile.vehicle.type;
    let seats = driverProfile.vehicle.seats || 4;
    let pricePerKm = driverProfile.pricePerKm || 5;

    // For scooter/bike, enforce 1 seat only (price is not enforced, just recommended)
    if (vehicleType === "scooter/bike") {
      seats = 1; // Always 1 for two-wheelers
    }
...
exports.updateDriverVehicleAndPricing = async (req, res) => {
  try {
    ...
    if (vehicle) {
      // Store the old vehicle type to check if it changed
      const oldVehicleType = user.driverProfile.vehicle.type;
      const newVehicleType = vehicle.type || oldVehicleType;
      ...
      // Handle seat changes based on vehicle type
      if (newVehicleType === "scooter/bike") {
        // For scooter/bike, always set seats to 1
        user.driverProfile.vehicle.seats = 1;
      } else if (newVehicleType === "car") {
        // For car, use provided seats or default to 4 if switching from scooter/bike
        if (oldVehicleType === "scooter/bike" && !vehicle.seats) {
          user.driverProfile.vehicle.seats = 4;
        } else if (vehicle.seats) {
          // Validate seats for car (1-6)
          const seats = parseInt(vehicle.seats);
          if (seats < 1 || seats > 6) {
            return res.status(400).json({ 
              message: "Number of seats must be between 1 and 6 for cars" 
            });
          }
          user.driverProfile.vehicle.seats = seats;
        }
      }
    }
    ...
    if (pricePerKm !== undefined) {
      // Validate price per km (1-10)
      const price = parseFloat(pricePerKm);
      if (price < 1 || price > 10) {
        return res.status(400).json({ 
          message: "Price per km must be between ₹1 and ₹10" 
        });
      }
      user.driverProfile.pricePerKm = price;
    }
...
exports.updateHitcherProfile = async (req, res) => {
  try {
    const userId = req.user.id;
    const { phone, homeAddress, homeCoordinates, gender, distanceToCollege } = req.body;
