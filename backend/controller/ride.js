...
exports.createRide = async (req, res) => {
  try {
    // Get driver's vehicle type from their profile
    ...
    // Add vehicle type to ride data
    const rideData = {
      ...req.body,
      vehicleType: driver.driverProfile.vehicle?.type || null
    };
    ...
    logger.info('Ride created', {
      ...
      date: savedRide.date,
      direction: savedRide.direction,
      from: savedRide.from,
      to: savedRide.to,
      availableSeats: savedRide.availableSeats,
      pricePerKm: savedRide.pricePerKm,
    ...
    logger.error("Error creating ride", { error: err.message, stack: err.stack });
...
exports.getRides = async (req, res) => {
  try {
    ...
    const rides = await Ride.find(query)
      .populate("driver", "name email phone gender srn college homeCoordinates driverProfile.vehicle.model driverProfile.vehicle.type driverProfile.reliabilityRate")
      .populate("hitchers.user", "name email phone gender srn college homeCoordinates hitcherProfile.reliabilityRate")
...
exports.requestRide = async (req, res) => {
  try {
    ...
    logger.error("Error requesting ride", { error: err.message, stack: err.stack });
...
exports.acceptRide = async (req, res) => {
  try {
    ...
    const ride = await Ride.findById(rideId).populate('hitchers.user', 'email name').populate('driver', 'name phone');
    if (!ride) {
      return res.status(404).json({ message: "Ride not found" });
    }
    ...
    // Check if the hitcher is already accepted
    ...
    // If this is a scooter/bike (1 seat only) or no seats left, auto-reject all other pending requests for this ride
    if (ride.vehicleType === 'scooter/bike' || ride.availableSeats === 0) {
      const rejectedEmails = [];
      
      ride.hitchers.forEach((h) => {
        if (h.user && h.user._id.toString() !== hitcherId && h.status === 'pending') {
          h.status = 'auto-rejected';
          h.autoCancel = true;
          
          // Collect email for notification
          if (h.user.email) {
            rejectedEmails.push(h.user.email);
          }
          
          // Add notification for each rejected hitcher
          if (!ride.notifications) {
            ride.notifications = [];
          }
          ...
          ride.notifications.push({
            userId: h.user._id,
            message: `Your ride request was automatically cancelled because all seats are now filled`,
            read: false,
            createdAt: new Date()
          });
          
          logger.info('Auto-rejected pending request due to full capacity', {
            rideId: rideId,
            rejectedHitcherId: h.user._id.toString(),
      ...
      // Send email notifications to all rejected hitchers
      if (rejectedEmails.length > 0) {
        message = `Your ride request was automatically cancelled because all seats were filled. The driver accepted another hitcher and there's no more space available. Please search for another ride at ${process.env.CLIENT_URL}`;
        sendEmailNotification({message, email: rejectedEmails.join(',')});
      }
    }
    ...
    const driverPhone = ride.driver.phone || 'Not provided';
        
    message = `${driverFirstName} has accepted your ride request. You can contact ${driverFirstName} at ${driverPhone} to coordinate pickup details. Visit ${process.env.CLIENT_URL} to view the ride details and driver profile.`;
...
exports.rejectRide = async (req, res) => {
  try {
    ...
    const ride = await Ride.findById(rideId).populate('hitchers.user', 'email name').populate('driver', 'name');
    if (!ride) {
      return res.status(404).json({ message: "Ride not found" });
    }
    ...
    // Log ride rejection
    logger.info('Ride request rejected', {
      rideId: rideId,
      hitcherId: hitcherId,
      hitcherName: hitcher.user.name,
      driverId: ride.driver._id,
    ...
    const hitcherEmail = hitcher.user.email;
    message = `Your ride request was rejected by the driver, visit ${process.env.CLIENT_URL} for details`;
    sendEmailNotification({message, email : hitcherEmail});
  } catch (err) {
    logger.error("Error declining ride", { error: err.message, stack: err.stack });
    console.error("Error declining ride:", err.message);
    res.status(500).json({
...
exports.calculateReliabilityImpact = async (req, res) => {
  try {
    ...
    console.error("Error calculating reliability impact:", error.message);
...
exports.getHitcherStatus = async (req, res) => {
  try {
    ...
    console.error('Error getting hitcher status:', error.message);
    res.status(500).json({ 
      success: false, 
...
exports.getRideById = async (req, res) => {
  try {
    ...
    const ride = await Ride.findById(rideId)
      .populate("driver", "name email phone gender srn college homeCoordinates driverProfile.vehicle.model driverProfile.vehicle.registrationNumber driverProfile.reliabilityRate")
      .populate("hitchers.user", "name email phone gender srn college homeCoordinates hitcherProfile.reliabilityRate");
